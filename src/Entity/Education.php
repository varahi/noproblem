...
class Education
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=Job::class, mappedBy="education")
     */
    private $jobs;

    /**
     * @ORM\OneToMany(targetEntity=Worksheet::class, mappedBy="education")
     */
    private $worksheets;
    ...
    public function removeWorksheet(Worksheet $worksheet): self
    {
        if ($this->worksheets->removeElement($worksheet)) {
            // set the owning side to null (unless already changed)
            if ($worksheet->getEducation() === $this) {
                $worksheet->setEducation(null);
            }
        }
